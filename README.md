...
- **The random measure**: `poissonRandomMeasure` is a genuine `Measure`-valued random object, a countable sum of Dirac masses at the realized points, with `poissonRandomMeasure_apply` its evaluation and `measurable_poissonRandomMeasure_apply` measurability; `map_poissonRandomMeasure_apply` is the superposition law — the count `N(A)` on a finite-mass set is Poisson with mean `m A` — and `indepFun_poissonRandomMeasure_apply` gives pairwise independence of the evaluations on disjoint finite-mass sets (mutual independence of a finite family of disjoint-set evaluations is not yet formalized)
